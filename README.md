...
当前推荐使用 `v0.13.0`。这一版把首次使用流程改成“局域网房间大厅”：可以扫描附近房间、选房间后输入密码加入，也可以在设置页退出当前房间后重新加入。核心聊天、文件、终端、看屏和远控能力保持兼容。
...
```text
release/Lan-Control-Hub-0.13.0-win-x64-portable.exe
release/Lan-Control-Hub-0.13.0-win-x64-setup.exe
```
...
git status
git add .
git commit -m "prepare v0.13.0 release"
git push
```
