...
- 会话基础层：聊天事件保留旧的一对一 key，同时维护会话 metadata，为后续群聊和多人会话迁移做准备。
...
```text
Lan-Control-Hub-0.9.0-win-x64-portable.exe
Lan-Control-Hub-0.9.0-win-x64-setup.exe
Lan-Control-Hub-0.9.0-mac-x64.zip
Lan-Control-Hub-0.9.0-mac-arm64.zip
SHA256SUMS.txt
```
...
每台设备的聊天/文件会话默认保留最近 1000 条事件，超过后自动裁剪旧记录。聊天支持本地搜索、回复引用、基础 reaction，以及 `**加粗**`、`` `代码` `` 和 fenced code block 这类安全的 Markdown 子集。当前版本会同时维护会话 metadata：旧的一对一会话 key 不变，后续群组/多人会话会基于这层 metadata 继续演进。收到大量消息可以正常接收，但当前不是企业 IM，不做云端同步和无限历史归档。
...
```text
release/Lan-Control-Hub-0.9.0-win-x64-portable.exe
release/Lan-Control-Hub-0.9.0-win-x64-setup.exe
```
