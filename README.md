...
当前推荐使用 `v0.14.1`。这一版在 `v0.14.0` 的手机 Web/PWA 控制台基础上，整理了桌面端工作台和设置页：设备能力显示为用户能理解的标签，手机控制台入口更明显，Local API 和防火墙修复收进高级工具。

从 GitHub Releases 下载和自己设备匹配的包：

```text
Lan-Control-Hub-0.14.1-win-x64-portable.exe
Lan-Control-Hub-0.14.1-win-x64-setup.exe
Lan-Control-Hub-0.14.1-mac-x64.zip
Lan-Control-Hub-0.14.1-mac-arm64.zip
SHA256SUMS.txt
```
...
```text
release/Lan-Control-Hub-0.14.1-win-x64-portable.exe
release/Lan-Control-Hub-0.14.1-win-x64-setup.exe
```
...
git status
git add .
git commit -m "prepare v0.14.1 release"
git push
```
