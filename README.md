...
- 手机控制台：已加入房间的电脑会在 `46882/mobile/` 提供移动端 Web/PWA。默认是基础控制面板；预设命令、任意命令和跨设备批量执行需要在桌面端手动开启 Agent Gateway 高级模式。
...
手机端可以打开任意一台已加入房间电脑的 `http://电脑IP:46882/mobile/`。离开局域网后，手机可以通过 Tailscale 访问一台网关电脑。默认基础模式只提供设备状态、任务查看和网关本机快捷动作；如果确实需要通过网关电脑操作其它已信任设备，先在桌面端“设置 / 系统 / 高级工具”开启 Agent Gateway。开启后，手机端会显示命令页，可选择“网关本机 / 选中设备 / 全部在线”，语音识别会先填入命令或匹配快捷动作，执行前仍需确认。详细边界见 [docs/移动端控制台.md](docs/移动端控制台.md)。
