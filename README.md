...
- 手机控制台：已加入房间的电脑会在 `46882/mobile/` 提供移动端 Web/PWA，手机可在局域网或 Tailscale 中使用快捷动作、语音填入和确认式命令控制网关或多台设备。
...
当前推荐使用 `v0.15.0`。这一版重做了手机 Web/PWA 控制台：手机端有总览、设备、命令、任务四个页签，并支持语音填入命令、网关本机命令、选中设备命令和全部在线设备命令。

从 GitHub Releases 下载和自己设备匹配的包：

```text
Lan-Control-Hub-0.15.0-win-x64-portable.exe
Lan-Control-Hub-0.15.0-win-x64-setup.exe
Lan-Control-Hub-0.15.0-mac-x64.zip
Lan-Control-Hub-0.15.0-mac-arm64.zip
SHA256SUMS.txt
```
...
手机端可以打开任意一台已加入房间电脑的 `http://电脑IP:46882/mobile/`。离开局域网后，手机可以通过 Tailscale 访问一台网关电脑，再由这台网关电脑通过 Lan Control Hub 的局域网控制通道操作其它已信任设备；这不是网络层自动桥接，而是应用层中转。新版手机端提供总览、设备、命令和任务四个页签，命令页可以选择“网关本机 / 选中设备 / 全部在线”，语音识别会先填入命令或匹配快捷动作，执行前仍需确认。详细边界见 [docs/移动端控制台.md](docs/移动端控制台.md)。
...
```text
release/Lan-Control-Hub-0.15.0-win-x64-portable.exe
release/Lan-Control-Hub-0.15.0-win-x64-setup.exe
```
...
git status
git add .
git commit -m "prepare v0.15.0 release"
git push
```
