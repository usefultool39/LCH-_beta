...
- 聊天增强：支持本地搜索、回复引用、Markdown/代码块渲染、基础 reaction 和第一版群组文本会话，旧设备会降级为普通文本。
- 会话基础层：聊天事件保留旧的一对一 key，同时维护会话 metadata，群组消息会按成员逐个通过现有加密通道发送。
...
```text
Lan-Control-Hub-0.10.0-win-x64-portable.exe
Lan-Control-Hub-0.10.0-win-x64-setup.exe
Lan-Control-Hub-0.10.0-mac-x64.zip
Lan-Control-Hub-0.10.0-mac-arm64.zip
SHA256SUMS.txt
```
...
每台设备的聊天/文件会话默认保留最近 1000 条事件，超过后自动裁剪旧记录。聊天支持本地搜索、回复引用、基础 reaction，以及 `**加粗**`、`` `代码` `` 和 fenced code block 这类安全的 Markdown 子集。聊天页左侧会显示直接会话和群组会话；群组文本消息会携带 `conversationId`、成员和标题等可选 metadata，再通过现有加密控制通道逐个发送给成员。旧的一对一会话 key 不变，旧设备不认识群组 metadata 时仍按普通文本消息处理。群组文件发送、已读回执和云端同步不是当前版本目标。
...
```text
release/Lan-Control-Hub-0.10.0-win-x64-portable.exe
release/Lan-Control-Hub-0.10.0-win-x64-setup.exe
```
...
git status
git add .
git commit -m "prepare v0.10.0 release"
git push
```
