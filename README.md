...
当前推荐使用 `v0.15.2`。这一版修复文件中枢只能看到常用目录的问题：目标电脑开启“完整磁盘访问”后，可信设备可以从远程文件页看到 `C:`、`D:` 和外接盘等可见磁盘。
...
```text
release/Lan-Control-Hub-0.15.2-win-x64-portable.exe
release/Lan-Control-Hub-0.15.2-win-x64-setup.exe
```
...
git status
git add .
git commit -m "prepare v0.15.2 release"
git push
```
